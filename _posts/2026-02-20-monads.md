...
This is a level 0 monad. It does the job. But it has a few shortcomings. Firstly, C# does not have the language features required to define the actual monad contract. So aside from the name we have no way to know that this type really is a monad.
...
which C# does not allow. So despite all the monads kind of being similar, from a programmatic perspective there is no way to verify this at compile time.
...
Programmers proficient in dependent typing make no such assumptions. All claims, especially those about equality, must be backed up by proofs. That is why the true monad looks like this:
